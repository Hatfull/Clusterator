...
def getblastrecords(InputFile): 
	...
	(mycobacteriophages599 in this version), and returns the blast records
	"""
	OutputFile = InputFile[:-6] + ".xml"
	blastn_cline = NcbiblastnCommandline(query=InputFile, db="mycobacteriophages599", evalue=10, outfmt=5, out=OutputFile, reward=3, penalty=-2, gapopen=5, gapextend=5)
...
if __name__ == '__main__':
	...
	if len(blast_record.alignments) == 0:
		...
		second_hit_length = second_alignment_data.length
		
		if int(second_alignment_data.hsps[1].positives) == int(second_hit_length):
			print "Your genome is 100%% identical to %s and is probably the same genome" % top_hit_name

		else:		
			print "Top hit is to %s (%s, %s) percent span length hit: %s" % (second_hit_name, second_cluster_assignment[0], second_cluster_assignment[1], second_hit_percent)
